...
function findIndentedBlock(content, name, keyword) {
  ...
  // Walk endLine back past any trailing blank lines that belong to the inter-
  // function separator, not to this block.  Without this, replaceMethod /
  // deleteMethod consume the PEP 8 blank lines, collapsing the gap between
  // adjacent top-level or class-level functions (flake8 E302 / E303).
...
const METHOD_PATTERNS = [
  ...
  // (?:\s*:\s*[^{]+)? allows an optional TypeScript return-type annotation between
  // ) and { — e.g. `export default function foo(x: T): error is TypeError {`
  // Without this, TS-annotated functions fall through to the typed-declaration
  // pattern which matches mid-word and produces duplicated export default.
  `(?:(?:export\\s+default\\s+|export\\s+|async\\s+|static\\s+)*)function\\s+__NAME__\\s*\\([^)]*\\)(?:\\s*:\\s*[^{]+)?\\s*\\{`,
