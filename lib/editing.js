...
//   • Python triple-quoted strings are treated as three adjacent single-quoted
//     strings — may leave a small residue, but will not cause false positives in
//     brace-counting or word-boundary symbol matching.
...
// Bug 1 fix: METHOD_PATTERNS patterns are now applied against the comment/string-
// masked source (via findCurlyBlockByNameSafe) so a method name that appears
// only inside a doc-comment or string literal can never cause a false match.
//
// The first pattern previously had `[^;{=]*` on the parameter list which allows
// comment text to bleed into the match when the comment sits between the
// method name and the opening brace.  The fix tightens the parameter clause to
// `[^;{}\n]*` (no newlines or braces) so the match must stay on the signature
// lines and cannot swallow an arbitrary comment block.
const METHOD_PATTERNS = [
  ...
  // Go method
  `func\\s*(?:\\([^)]*\\)\\s*)?__NAME__\\s*\\([^)]*\\)[^\\{\\n]*\\{`,
  // Arrow / const assignment
  `(?:const|let|var)\\s+__NAME__\\s*=\\s*(?:async\\s*)?(?:\\([^)]*\\)|\\w+)\\s*=>\\s*\\{`,
];
