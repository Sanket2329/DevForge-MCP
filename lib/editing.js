...
const METHOD_PATTERNS = [
  ...
  // Plain function declaration — optionally preceded by export default / export / async / static
  `(?:(?:export\\s+default\\s+|export\\s+|async\\s+|static\\s+)*)function\\s+__NAME__\\s*\\([^)]*\\)\\s*\\{`,
  // Go method
  `func\\s*(?:\\([^)]*\\)\\s*)?__NAME__\\s*\\([^)]*\\)[^\\{\\n]*\\{`,
  // Arrow / const assignment — optionally preceded by export / export default
