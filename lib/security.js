...
function rateLimiter({ windowMs = 60000, max = 120 } = {}) {
  ...
  // Bug 9 fix: prune stale entries once per window to prevent unbounded Map
  // growth when many unique IPs each make a single request and never return.
  // Runs at most once per windowMs interval.
  let lastPruneAt = Date.now();
  function pruneIfDue(now) {
