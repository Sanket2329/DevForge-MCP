...
function gitDiff(cwd, file) {
  const args = ["diff"];
  if (file) {
    // Bug 11 fix: reject absolute paths and any path that escapes the working dir,
    // not just ".." traversal.  git itself won't leak content outside the repo but
    // the validation was misleadingly weak and inconsistent with resolveInsideProject.
    if (
      typeof file !== "string" ||
      file.includes("..") ||
      file.startsWith("/") ||
      /^[a-zA-Z]:[/\\]/.test(file) // Windows absolute: C:\ or C:/
    ) {
