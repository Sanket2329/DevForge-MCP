...
/**
 * detectArchitecture — analyse the codebase index and return an architecture profile.
 *
 * Bug 13 fix: accepts optional { projectPath, fsTools } so it reads file content
 * fresh from disk rather than from the 25 KB-capped index cache.  Pattern
 * detection (imports, namespace, tech stack) that lives past the 25 KB mark of
 * large files is no longer silently missed.  Falls back to the cached slice when
 * the disk read fails.
 *
 * @param {object} index
 * @param {object} [opts]
 * @param {string} [opts.projectPath]
 * @param {object} [opts.fsTools]   — lib/fsTools (needs readFileSafe)
 */
function detectArchitecture(index, { projectPath, fsTools } = {}) {
  const allPaths = Object.keys(index);

  // Build allContent from full disk reads when possible (Bug 13 fix).
  const allContent = allPaths.map((relPath) => {
    if (projectPath && fsTools) {
      const read = fsTools.readFileSafe(projectPath, relPath);
      if (read.success) return read.content;
    }
    return index[relPath].content; // fallback to cached slice
  }).join("\n");
