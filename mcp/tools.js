...
function getToolDefinitions(MEMORY_TYPES) {
  ...
  return [
    ...
    { name: "get_file_content", description: "Read a single source file by relative path. Reads directly from disk — never truncated, always returns the full file content.", inputSchema: { type: "object", properties: { path: { type: "string" } }, required: ["path"] }, annotations: RO },
    ...
    { name: "rename_symbol", description: "Rename a symbol everywhere it's used across the indexed codebase (word-boundary match, comment/string-safe) and write every changed file back to disk. Use scope='file' with rel_path to rename within a single file only (safer). scope='project' (default) renames across all files — always confirm with the user before using project scope.", inputSchema: { type: "object", properties: { old_name: { type: "string" }, new_name: { type: "string" }, ext_filter: { type: "string" }, scope: { type: "string", enum: ["file", "project"], description: "Rename scope. 'file' requires rel_path. Default: 'project'." }, rel_path: { type: "string", description: "Required when scope='file'. Relative path of the single file to rename within." } }, required: ["old_name", "new_name"] } },
